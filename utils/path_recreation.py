def get_path_from_predecessor(target_nodes, predecessors, reversal = False):  # outputs in format {node: path to node}
    ...
    by x.dijkstra_predecessor_and_distance.
    ...
    paths_dict = {}
    # find out the paths for each of the nodes in the targe dict
    for node in target_nodes:

        current_node = node

        paths_dict[node] = [node]  # Initializing list for this node  adding the first node itself
