...
def generate_random_filtered_coordinates(number, seed, G, min_street_count):
    """
        ...
        :param seed: seed
        :param G: graph to be processed
        :param min_street_count: min number of streets for the node to be acceptable, as defined by osmnx
        :return: list of tuples
        """
    ...
    while (counter !=number):
        ...
        node_street_count = G.nodes[random_node[0]]['street_count']
        if node_street_count >= min_street_count:
            selected_nodes.append(random_node[0])
            counter += 1
            print( G.nodes[random_node[0]]['street_count'])
