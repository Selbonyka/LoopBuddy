...
def ring_s_and_sprime_handling(G, point_s,bounds_Rs_penalized, bounds_Rs_prime_traversing, bounds_Rs):
    ...
    R_s_penalized = {node for node, dist in penalized_distances.items() if bounds_Rs_penalized[0] <= dist <= bounds_Rs_penalized[1]}  # getting out the Ring S, which is the nodes in distances inside the penalizined error bounds
